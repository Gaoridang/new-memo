...
// 약속·일정('오늘 오후 6시 저녁약속')은 기준에 적기 전에는 0.5~0.8에 걸려 바뀌지 않았다. 약속 28줄과 날짜·시간이
// 있지만 할 일이 아닌 줄 30줄(지난 일, 예보, 되풀이되는 일정 규칙)로 다시 확인했다. 약속은 0.85 이상
// (제목이 '일기'면 몇 줄은 0.35~0.55), 할 일이 아닌 줄은 0.25 이하.
const KIND_QUESTION = {
  type: 'choice',
  instructions:
    '메모 앱의 한 줄(`line`)이 어떤 종류인지 고른다. `line`만 판단하고, `memo_title`과 `nearby_lines`는 맥락으로만 참고한다.',
  criteria: {
    todo: '작성자가 직접 해야 할 일. 동사가 있는 문장("세탁소 들르기", "call mom")뿐 아니라 "우유 사기", "치과 예약", "보고서 제출", "책 반납"처럼 명사 한두 개에 행동(사기·예약·제출·반납·전화)이 붙은 짧은 항목도 할 일이다. 앞으로 정해진 때에 작성자가 가거나 참석할 약속·일정·회의·모임·예약도 할 일이다("오늘 오후 6시 저녁약속", "금요일 팀 회식", "3시 회의", "dinner with Sam at 7")',
    heading:
      '행동이 아니라 목록이나 메모를 묶는 제목·소제목·분류 이름. 예: "장보기 목록", "이번 주 할 일", "회의록", "Groceries", "아이디어"',
    note: '사실, 감상, 생각, 일기, 이미 지나간 일, 남의 이야기, 날씨나 소식처럼 작성자가 할 행동이 아닌 기록("어제 저녁약속 재밌었다", "회의가 길었다"). 매주·매일처럼 되풀이되는 일정의 규칙("회의는 매주 월요일 10시에 한다"), 레시피 단계, 사용법, 설명서처럼 방법이나 절차를 적어 둔 줄도 여기에 속한다',
  },
};
